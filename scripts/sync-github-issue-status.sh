...
GitHub update options:
  --status VALUE           in-progress, failed, ready-for-review, or done (default: ready-for-review).
...
case "$STATUS" in
  in-progress|failed|ready-for-review|done) ;;
  *)
    echo "--status must be in-progress, failed, ready-for-review, or done, got: $STATUS" >&2
    exit 2
    ;;
esac
...
LABEL_CATALOG = {
    ...
    "continuum:failed": ("D1242F", "Catalyst Continuum run failed before review-ready evidence."),
...
def default_summary() -> str:
    ...
    if status == "failed":
        return (
            "Catalyst Continuum attempted this issue work package, but the local developer flow "
            "failed before it produced review-ready evidence."
        )
