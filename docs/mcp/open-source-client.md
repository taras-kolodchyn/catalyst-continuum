...
- env: optional `CATALYST_DATABASE_URL`, `CATALYST_RUNTIME_PROVIDERS_FILE`, and `CATALYST_MCP_SERVERS_FILE`

Use `--runtime-providers-file` when the agent should target a non-default runtime-provider config path, and `--mcp-servers-file` when the same session should use an explicit external MCP server allowlist. That keeps MCP, HTTP, and local worker execution aligned to the same instance contract during validation.
