...
the issue package with labels and recipes, and provides copy buttons for the next review command and
the issue-sync apply command. It is intentionally read-only, so GitHub still changes only when you
run the copied command from a terminal.
