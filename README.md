...
The same instance-aware entrypoints also accept `--runtime-providers-file <path>`, `--mcp-servers-file <path>`, `--ai-gateway-file <path>`, and `--repository-targets-file <path>` when you need `serve`, `mcp-server`, or `describe-instance-config` to use explicit config files instead of relying only on environment discovery.
