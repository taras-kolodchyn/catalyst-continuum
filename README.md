...
The selected-run surface is now also narrative instead of table-first: it explains which orchestration stage is active, what the control plane already materialized, what remains blocked, and which operator action should happen next, so a first-time operator can follow the brief-to-run-to-quality-to-draft-PR flow without reverse-engineering raw run state.
