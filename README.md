...
`list_run_events` is the shared MCP audit path for run and task transitions, so an agent can inspect status changes, task starts/completions, and policy/quality checkpoints without inferring state from artifact timestamps alone. `claim_next_agent_task`, `prepare_agent_task_workspace`, `heartbeat_agent_task`, and `complete_agent_task` are the agent-owned execution handoff: they let OpenHands or another external executor atomically claim the next runnable task for its assignment, prepare a real run-scoped workspace for that task, refresh its reclaim lease during longer sessions, bind that work to an optional `executor_id`, and persist an `agent_task_report` artifact when the task is reported back as succeeded or failed. The claim response now also echoes the run-level `external_mcp_contract`, so the executor can see the current allowed or denied external MCP servers and pinned launch contracts without fetching `agent_dispatch_plan` first.
